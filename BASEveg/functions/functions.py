...
def IDW_interpolator(
    ...
    for ii in range(0,len(cell_wse)):
        ...
        #cell_interpolatedValue[ii] = cell_interpolatedValue[ii]-1
    print("--DONE--")
...
def exponential_decay(xi,xi_max,sigma,deltat):
    """
    This calculates the exponential decay given the parameters and initial state
    """
    delta_xi = (-1)*deltat*sigma*xi
    return delta_xi
...
def advance_in_time(var0,varmax,duration,deltat,formula,growth_rate):
    ...
    while exitcode:
        ...
        elif formula == 'exp_decay':
            deltavar = exponential_decay(var,varmax,growth_rate,deltat)
        else:
            print(f'Only formulas are ..exp.. , ..logistic.. , ...exp_decay...')
            break
        # advance value of var and time
        var += deltavar
        time += deltat 

        if time>duration and exitcode:     # adjust last time step
            newdeltat = duration - (time - deltat)
            time = duration
            deltat = newdeltat
            exitcode = False
        elif time>duration and exitcode==False:
            break
